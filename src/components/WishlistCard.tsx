...
export const WishlistCard: React.FC<WishlistCardProps> = ({ wishlist, onLike, onView }) => {
  const [isLiked, setIsLiked] = React.useState(wishlist.isLiked || false);
  const [likes, setLikes] = React.useState(wishlist.likes || 0);
  const [views, setViews] = React.useState(wishlist.views || 0);
  const [comments, setComments] = React.useState(wishlist.comments || 0);
  ...
  return (
    <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden group cursor-pointer animate-fade-in" onClick={handleView}>
      {/* Header */}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <button
              ...
              className={`flex items-center space-x-1 transition-all duration-200 hover:scale-110 ${isLiked ? 'text-red-500' : 'text-neutral-500 hover:text-red-500'} ${loadingLike ? 'opacity-60 cursor-not-allowed' : ''}`}
            >
