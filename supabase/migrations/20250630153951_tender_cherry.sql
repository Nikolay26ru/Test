/*
  ...
  1. Создание всех необходимых таблиц и структур
  2. Исправление foreign key constraints
  3. Настройка RLS политик для безопасности
  4. Добавление системы логирования и email
  5. Создание функций для автоматизации
*/
...
-- Создаем новые RLS политики используя только публичные функции
...
CREATE POLICY "wishlist_items_all_policy" ON wishlist_items
  FOR ALL USING (
    EXISTS (
      ...
      AND (SELECT auth.uid()) = wishlists.user_id
    )
  );
  ...
  FOR SELECT USING (
    EXISTS (
      ...
      AND (
        ...
        ((SELECT auth.uid()) IS NOT NULL AND wishlists.user_id = (SELECT auth.uid()))
      )
    )
  );
...
CREATE POLICY "campaigns_select_policy" ON campaigns
  FOR SELECT USING (
    EXISTS (
      ...
      AND (
        w.is_public = true OR 
        ((SELECT auth.uid()) IS NOT NULL AND w.user_id = (SELECT auth.uid()))
      )
    )
  );
...
-- Политики для friendships (если таблица существует)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'friendships') THEN
    EXECUTE 'CREATE POLICY "friendships_select_policy" ON friendships
      FOR SELECT USING ((SELECT auth.uid()) = user_id OR (SELECT auth.uid()) = friend_id)';
    
    EXECUTE 'CREATE POLICY "friendships_insert_policy" ON friendships
      FOR INSERT WITH CHECK ((SELECT auth.uid()) = user_id)';
    
    EXECUTE 'CREATE POLICY "friendships_delete_policy" ON friendships
      FOR DELETE USING ((SELECT auth.uid()) = user_id OR (SELECT auth.uid()) = friend_id)';
  END IF;
END $$;

-- Политики для friend_requests (если таблица существует)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'friend_requests') THEN
    EXECUTE 'CREATE POLICY "friend_requests_select_policy" ON friend_requests
      FOR SELECT USING ((SELECT auth.uid()) = sender_id OR (SELECT auth.uid()) = receiver_id)';
    
    EXECUTE 'CREATE POLICY "friend_requests_insert_policy" ON friend_requests
      FOR INSERT WITH CHECK ((SELECT auth.uid()) = sender_id)';
    
    EXECUTE 'CREATE POLICY "friend_requests_update_policy" ON friend_requests
      FOR UPDATE USING ((SELECT auth.uid()) = receiver_id OR (SELECT auth.uid()) = sender_id)';
    
    EXECUTE 'CREATE POLICY "friend_requests_delete_policy" ON friend_requests
      FOR DELETE USING ((SELECT auth.uid()) = sender_id OR (SELECT auth.uid()) = receiver_id)';
  END IF;
END $$;

-- Политики для product_views (если таблица существует)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'product_views') THEN
    EXECUTE 'CREATE POLICY "product_views_select_policy" ON product_views
      FOR SELECT USING ((SELECT auth.uid()) = user_id)';
    
    EXECUTE 'CREATE POLICY "product_views_insert_policy" ON product_views
      FOR INSERT WITH CHECK ((SELECT auth.uid()) = user_id)';
  END IF;
END $$;

-- Политики для product_recommendations (если таблица существует)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'product_recommendations') THEN
    EXECUTE 'CREATE POLICY "product_recommendations_select_policy" ON product_recommendations
      FOR SELECT USING ((SELECT auth.uid()) = user_id)';
    
    EXECUTE 'CREATE POLICY "product_recommendations_all_policy" ON product_recommendations
      FOR ALL USING ((SELECT auth.uid()) = user_id)';
  END IF;
END $$;
...
-- Функции для работы с дружбой (если таблицы существуют)
