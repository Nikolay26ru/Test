/*
  # Исправление схемы базы данных WishFlick

  1. Новые таблицы
    - `operation_logs` - логирование операций
    - `email_queue` - очередь email сообщений  
    - `email_templates` - шаблоны писем

  2. Исправления существующих таблиц
    - Добавление недостающих колонок в `profiles`
    - Исправление foreign key связей
  ...
  4. Триггеры и функции
    ...
    - Функции для работы с email очередью
    - Функции для дружеских связей
*/
