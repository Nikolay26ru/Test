/*
  ...
  2. Обновления существующих таблиц
    - `wishlists` - добавлены категории и теги
    - `wishlist_items` - добавлено поле purchased_by
...
DO $$
BEGIN
  -- Добавляем category и tags в wishlists если их нет
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'wishlists' AND column_name = 'category'
  ) THEN
    ALTER TABLE wishlists ADD COLUMN category text;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'wishlists' AND column_name = 'tags'
  ) THEN
    ALTER TABLE wishlists ADD COLUMN tags text[];
  END IF;

  -- Обновляем внешний ключ wishlists.user_id на profiles
  IF EXISTS (
  ...
  -- Добавляем purchased_by в wishlist_items если его нет
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'wishlist_items' AND column_name = 'purchased_by'
  ) THEN
    ALTER TABLE wishlist_items ADD COLUMN purchased_by uuid REFERENCES profiles(id);
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Пользователи могут обновлять свой" ON profiles;
DROP POLICY IF EXISTS "Пользователи могут создавать проф" ON profiles;
...
DROP POLICY IF EXISTS "Users can view items from accessible wishlists" ON wish_items;
DROP POLICY IF EXISTS "Users can manage items in own wishlists" ON wish_items;
...
-- Триггеры для updated_at (удаляем существующие перед созданием)
